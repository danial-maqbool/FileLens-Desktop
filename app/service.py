...
class Application(BaseApplication):
    ...
    def state(self):
        ...
        extensions = self.store.rows(
            "SELECT extension,COUNT(*) AS count FROM files GROUP BY extension ORDER BY count DESC"
        )
        return {
            "stats": stats,
            "roots": self.store.rows("SELECT * FROM roots ORDER BY name"),
            "extensions": extensions,
            "saved": self.store.rows("SELECT * FROM saved_searches ORDER BY name"),
            "semantic_model_path": self.store.get("semantic_model_path", ""),
    ...
    def get(self, action, query):
        if action == "search":
            ...
            return self.index.ranked_search(
                str(query.get("q", ""))[:500],
    ...
    def post(self, action, body):
        if action == "semantic/model":
            ...
            return {
                "saved": True,
                "model": (
                    "Local neural weights" if path else "Local latent semantic analysis"
                ),
            }
        if action == "roots/add":
            return self.index.add_root(
                body.get("path", ""), body.get("patterns"), bool(body.get("ocr", False))
            )
        ...
        if action == "scan":
            return {
                "job_id": self.scan(
                    integer(body.get("id"), 1, 10**9), bool(body.get("force", False))
                )
            }
        if action == "tags":
            ...
            self.store.execute(
                "UPDATE files SET tags=? WHERE id=?", (json.dumps(tags), ident)
            )
            return {"tags": tags}
        if action == "saved/add":
        ...
        if action == "saved/delete":
            self.store.execute(
                ...
                (integer(body.get("id"), 1, 10**9),),
            )
            return {"deleted": True}
        if action == "export":
            result = self.index.ranked_search(
                str(body.get("query", ""))[:500],
                mode=str(body.get("mode", "keyword")),
                extension=str(body.get("extension", "")),
                tag=str(body.get("tag", "")),
                root=integer(body["root"], 1, 10**9) if body.get("root") else None,
                limit=1000,
            )
            ...
            if body.get("format", "json") == "csv":
                ...
                writer.writerow(
                    ["Name", "Path", "Bytes", "Method", "Tags", "Coverage note"]
                )
                ...
                path = unique_write(
                    folder / "search-results.csv", stream.getvalue().encode("utf-8-sig")
                )
            else:
                path = unique_write(
                    folder / "search-results.json",
        ...
        if action == "watch/start":
            ...
            self.watch_thread = threading.Thread(
                target=loop, daemon=True, name="file-index-rescan"
            )

...
from localdesk.safety import (DEFAULT_EXCLUDES, InputError, MAX_FILE_BYTES, checked_path,
                             digest, walk_files, within)
...
class Index:
    def __init__(self, store, private_data: Path):
        ...
        with store.connection() as db:
            ...
            ''')
            try:
                db.execute('CREATE VIRTUAL TABLE IF NOT EXISTS search_index USING fts5(filename,path,body,tokenize="unicode61")')
                db.execute('CREATE VIRTUAL TABLE IF NOT EXISTS search_vocabulary USING fts5vocab(search_index,row)')
                self.fts = True
            except sqlite3.OperationalError:
                self.fts = False

    def add_root(self, value: str, patterns: list[str] | None = None, ocr: bool = False) -> dict:
        ...
        if not isinstance(patterns, list) or len(patterns) > 80 or any(not isinstance(p, str) or len(p) > 150 for p in patterns):
        ...
        for other in self.store.rows('SELECT path FROM roots'):
            if within(path, Path(other['path'])) or within(Path(other['path']), path):
                raise InputError('Indexed folders must not overlap. Remove the existing root first.')
        ident = self.store.execute('INSERT INTO roots(path,name,patterns,ocr) VALUES(?,?,?,?)',
    ...
    def remove_root(self, ident: int) -> None:
        with self.store.connection() as db:
            if self.fts:
                db.execute('DELETE FROM search_index WHERE rowid IN (SELECT id FROM files WHERE root_id=?)', (ident,))
    ...
    def scan(self, ident: int, context, *, force: bool = False) -> dict:
        ...
        files = [p for p in walk_files(folder, patterns=json.loads(root['patterns']))
        ...
        for offset, path in enumerate(files):
            ...
            source = str(path)
            seen.add(source)
            context.progress(2 + int(92 * offset / max(len(files), 1)), f'Indexing {path.name}')
            prior = self.store.one('SELECT id,mtime,size,sha256 FROM files WHERE path=?', (source,))
            if not force and prior and prior['mtime'] == stat.st_mtime_ns and prior['size'] == stat.st_size:
                skipped += 1
                continue
            ...
            if (stat.st_mtime_ns, stat.st_size) != (after.st_mtime_ns, after.st_size):
                warnings.append({'name': path.name, 'warning': 'File changed during indexing. Scan again.'})
                continue
            if warning:
                warnings.append({'name': path.name, 'warning': warning})
            with self.store.connection() as db:
                db.execute('INSERT INTO files(root_id,path,name,extension,size,mtime,sha256,text,method,warning,indexed) '
                           ...
                           (ident, source, path.name, path.suffix.lower(), stat.st_size, stat.st_mtime_ns,
                            sha, text, method, warning, utcnow()))
                file_id = db.execute('SELECT id FROM files WHERE path=?', (source,)).fetchone()[0]
                if self.fts:
                    db.execute('DELETE FROM search_index WHERE rowid=?', (file_id,))
                    db.execute('INSERT INTO search_index(rowid,filename,path,body) VALUES(?,?,?,?)',
                               (file_id, path.name, source, text))
            changed += 1
        context.check()
        # Only prune after a complete directory listing and processing pass.
        stale = [row['id'] for row in self.store.rows('SELECT id,path FROM files WHERE root_id=?', (ident,))
        ...
        with self.store.connection() as db:
            ...
            db.execute('UPDATE roots SET last_scan=?,last_error=? WHERE id=?',
                       (utcnow(), f'{len(warnings)} files need a coverage check.' if warnings else '', ident))
        return {'folder': str(folder), 'found': len(files), 'updated': changed, 'unchanged': skipped,
                'removed_from_index': len(stale), 'warnings': warnings, 'search_engine': 'FTS5' if self.fts else 'text match'}

    def search(self, query: str = '', *, extension: str = '', tag: str = '', root: int | None = None,
               limit: int = 100) -> dict:
        ...
        if tokens and self.fts:
            ...
            args.append(' AND '.join('"' + word.replace('"', '""') + '"*' for word in tokens))
            rank = 'bm25(search_index,3.0,0.15,1.0)'
        else:
            source, rank = 'files f', '0.0'
            for word in tokens:
                where.append('(lower(f.name) LIKE ? ESCAPE \'\\\' OR lower(f.text) LIKE ? ESCAPE \'\\\')')
        ...
        rows = self.store.rows(f'SELECT {fields},{rank} AS rank FROM {source}{clause} '
                              'ORDER BY rank ASC,f.name COLLATE NOCASE LIMIT ?', (*args, cap))
        output = []
        for row in rows:
            ...
            if tag and tag not in row['tags']:
                continue
            text = row.pop('text')
            matches = [m.start() for word in tokens for m in [re.search(re.escape(word), text, re.I)] if m]
            offset = max(0, min(matches) - 60) if matches else 0
            row['snippet'] = text[offset:offset + 260]
            row['match_reason'] = 'Filename match' if any(t in row['name'].casefold() for t in tokens) else (
                'Document text match' if tokens and matches else 'Path or metadata match' if tokens else 'Indexed file')
        ...
        if not output and tokens and self.fts:
            vocab = [r['term'] for r in self.store.rows('SELECT term FROM search_vocabulary ORDER BY cnt DESC LIMIT 5000')]
            for token in tokens:
                suggestions.extend(difflib.get_close_matches(token, vocab, n=3, cutoff=0.72))
        return {'results': output, 'suggestions': list(dict.fromkeys(suggestions)), 'limit': limit,
                'limited': len(output) >= limit, 'engine': 'SQLite FTS5' if self.fts else 'Plain text search'}

    def ranked_search(self, query: str = '', *, mode: str = 'hybrid', extension: str = '', tag: str = '', root: int | None = None, limit: int = 100) -> dict:
        if mode not in {'keyword', 'semantic', 'hybrid'}:
            raise InputError('Choose keyword, semantic, or hybrid search.')
        if not query.strip() or mode == 'keyword':
            return self.search(query, extension=extension, tag=tag, root=root, limit=limit)
        where, args = [], []
        if extension:
            where.append('extension=?'); args.append('.' + extension.lstrip('.').lower())
        if root is not None:
            where.append('root_id=?'); args.append(root)
        clause = ' WHERE ' + ' AND '.join(where) if where else ''
        records = self.store.rows('SELECT id,name,path,extension,size,method,warning,tags,indexed,text FROM files' + clause + ' ORDER BY id LIMIT 2001', tuple(args))
        if len(records) > 2000:
            raise InputError('Select a folder filter for semantic search across more than 2,000 files. Keyword search covers the full index.')
        for row in records:
            row['tags'] = json.loads(row['tags'])
        records = [r for r in records if not tag or tag in r['tags']]
        ranked = self.semantic_cache.search(records, query, model_path=self.store.get('semantic_model_path', ''), limit=limit)
        if mode == 'semantic':
            return ranked
        keyword = self.search(query, extension=extension, tag=tag, root=root, limit=limit)
        fused, rows = {}, {}
        ...
        combined = [{**rows[ident], 'fusion_score': round(fused[ident], 6)} for ident in sorted(fused, key=lambda ident: (-fused[ident], ident))[:limit]]
        return {**ranked, 'results': combined, 'engine': 'Keyword + ' + ranked['engine'],
                'suggestions': keyword['suggestions'], 'fusion': 'Reciprocal rank fusion, k=60'}
    ...
    def duplicates(self) -> dict:
        groups = self.store.rows("SELECT sha256,COUNT(*) AS count,MAX(size) AS size FROM files "
                                "WHERE sha256<>'' GROUP BY sha256 HAVING COUNT(*)>1 ORDER BY MAX(size) DESC LIMIT 100")
        for group in groups:
            group['files'] = self.store.rows('SELECT id,name,path FROM files WHERE sha256=? ORDER BY path',
                                             (group['sha256'],))
            group['extra_bytes'] = group['size'] * (group['count'] - 1)
        return {'groups': groups, 'extra_bytes': sum(g['extra_bytes'] for g in groups),
    ...
    def similar(self, ident: int) -> dict:
        ...
        result = self.ranked_search((source['name'] + ' ' + source['text'])[:4000], mode='semantic', limit=13)
        return {'results': [r for r in result['results'] if r['id'] != ident][:12],
    ...
    def word_similar(self, ident: int) -> dict:
        ...
        rows = self.store.rows('SELECT id,name,path,text FROM files WHERE id<>? AND extension=? LIMIT 200',
        ...
        return {'results': sorted(hits, key=lambda r: -r['word_overlap'])[:12],
